...
namespace bestdriver {
    ...
    void MapSystem::initNpcCars() {
        ...
        for (int hi = 0; hi < BLOCK_ROWS - 1 && (int)npcCars_.size() < 5; hi++) {
            ...
            npcCars_.push_back(NpcCar(cx, ry3, NPC_RIGHT, sp, c1[0], c1[1], c1[2]));
            if ((int)npcCars_.size() >= 5) break;
            // LEFT car: start further right in the map
            float cx2 = (float)(vRoadX_[3] + ROAD_W) + (float)(COL_W[4] / 2);
            const int* c2 = COLORS[ci++ % NC];
            npcCars_.push_back(NpcCar(cx2, ry1, NPC_LEFT, sp, c2[0], c2[1], c2[2]));
        }
        ...
        for (int vi = 1; vi < BLOCK_COLS - 1 && (int)npcCars_.size() < 10; vi += 2) {
            float cx1 = (float)(vRoadX_[vi] + 1) + 0.25f;           // down lane, away from center
            float cx3 = (float)(vRoadX_[vi] + 3) + 0.75f;   // up lane, away from center
            float cy = (float)(hRoadY_[0] + ROAD_W) + (float)(ROW_H[1] / 2);
            float sp = 3.5f + (rand() % 10) * 0.1f;
            const int* c1 = COLORS[ci++ % NC];
            npcCars_.push_back(NpcCar(cx1, cy, NPC_DOWN, sp, c1[0], c1[1], c1[2]));
            if ((int)npcCars_.size() >= 10) break;
            float cy2 = (float)(hRoadY_[1] + ROAD_W) + (float)(ROW_H[2] / 2);
            const int* c2 = COLORS[ci++ % NC];
            npcCars_.push_back(NpcCar(cx3, cy2, NPC_UP, sp, c2[0], c2[1], c2[2]));
        }
