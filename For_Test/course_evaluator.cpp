...
namespace bestdriver {
    ...
    void CourseEvaluator::checkTrafficSignals(const VehicleState& v, vector<Penalty>& pen) {
        ...
        // 1) 이미 lock된 신호가 있으면 그 신호만 계속 본다.
        ...
        // 2) lock된 신호가 없으면, 같은 축 신호 중 가장 그럴듯한 1개만 고른다.
        if (!targetSig) {
            int bestIdx = -1;
        ...
        // 너무 비스듬해서 확신이 낮으면 오탐 방지를 위해 그냥 판정 안 함
    ...
    void CourseEvaluator::checkSuddenMovement(const VehicleState& v, float dt, vector<Penalty>& pen) {
        if (dt <= 0.0f) return;
        ...
        if (rate > config_.suddenAccelThreshold && accelCooldown_ <= 0.0f) {
            appendPenalty(
                pen,
        ...
        if (rate < -config_.suddenBrakeThreshold && brakeCooldown_ <= 0.0f) {
            appendPenalty(
                pen,
